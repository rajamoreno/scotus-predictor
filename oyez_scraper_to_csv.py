...
class court_case:
    ...
    def get_first_party_label(self):
        first_party_label = self.json_data.get("first_party_label", {})
        if isinstance(first_party_label, str) and first_party_label != "":
            return first_party_label
        else:
            return None

    def get_first_party(self):
        first_party = self.json_data.get("first_party", {})
        if isinstance(first_party, str) and first_party != "":
            return first_party
        else:
            return None

    def get_second_party_label(self):
        second_party_label = self.json_data.get("second_party_label", {})
        if isinstance(second_party_label, str) and second_party_label != "":
            return second_party_label
        else:
            return None

    def get_second_party(self):
        second_party = self.json_data.get("second_party", {})
        if isinstance(second_party, str) and second_party != "":
            return second_party
        else:
            return None
    
    # not used for current task
    ...
    def get_winning_party(self):
        ...
        if isinstance(decisions, list) and len(decisions) > 0 and isinstance(decisions[0], dict):
            ...
            if isinstance(winning_party, str) and winning_party != "":
...
with open("2024-05-07-oyez-scrape.csv", "w", newline="") as write_file:

    csv_writer = csv.writer(write_file)
    csv_writer.writerow(["Case Key", "Case Name", "First Party Label", "First Party", "Second Party Label", "Second Party", "Winning Party", "Justices", "Facts", "Question", "Conclusion"])
    ...
    # for i in range(20): # enough to see sample behavior
        ...
        case_name = current_case.get_name()
        # print(case_name)
        case_first_party_label = current_case.get_first_party_label()
        # print(case_first_party_label)
        case_first_party = current_case.get_first_party()
        # print(case_first_party)
        case_second_party_label = current_case.get_second_party_label()
        # print(case_second_party_label)
        case_second_party = current_case.get_second_party()
        # print(case_second_party)
        case_winning_party = current_case.get_winning_party()
        # print(case_winning_party)
        ...
        if case_name and case_first_party_label and case_first_party and case_second_party_label and case_second_party and case_winning_party and case_justices and case_facts and case_question and case_conclusion:
            csv_writer.writerow([key, case_name, case_first_party_label, case_first_party, case_second_party_label, case_second_party, case_winning_party, case_justices_joined, case_facts, case_question, case_conclusion])
